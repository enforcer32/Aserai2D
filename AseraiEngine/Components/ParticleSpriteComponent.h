...
namespace Aserai
{
	struct ParticleSpriteComponent
	{
		...
		ParticleSpriteComponent(const glm::vec4& color = glm::vec4({ 1.0f, 0.0f, 0.0f, 1.0f }), uint32_t width = 0, uint32_t height = 0, uint32_t zIndex = 0, bool fixed = false)
		...
		ParticleSpriteComponent(const std::shared_ptr<Texture2D>& texture, uint32_t width = 0, uint32_t height = 0, uint32_t zIndex = 0, float textureX = 0, float textureY = 0, uint32_t textureW = 0, uint32_t textureH = 0, bool fixed = false)
